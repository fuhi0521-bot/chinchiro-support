...
class Style:
    ...
    # 鳴く最低打点。既定は 3900 だったが、混成卓（各6000席局）で 1500 に負けた。
    #   和了率 17.8% → 19.7%（+1.90pt、標準誤差 0.5pt）
    #   放銃率 10.2% → 10.1%（変わらない）
    #   素点/局 +162.5 → +232.5
    # 全員同じ設定の卓でも、3900 はテンパイ到達率が 67.0% → 60.7% に落ち、
    # 流局が 124 → 147 に増えていた。打点で門を上げると、速度だけでなく
    # 放銃率も悪くなる（テンパイできないまま他家のテンパイを受けるため）。
    call_min_value: int = 1500
    # 何シャンテンまで鳴くか。既定は 2 だったが、3 に負けた。
    #   和了率 18.2% → 19.4%（+1.13pt） / 放銃率 10.3% → 9.8%（−0.43pt）
    call_max_shanten: int = 3
    # タンヤオを当てにして鳴くとき、手に残っていてよい么九牌の枚数。
    # 残した么九牌はぜんぶ落とさないと役にならないので、多いほど外れる。
    #
    # 300局で振った実測（0 / 1 / 2）:
    #   鳴いた局        301 / 410 / 518
    #   役が乗った    91.7% / 82.2% / 71.8%   ← 2 だと28%が役なしのまま終局
    #   タンヤオ不発   0.0% / 20.4% / 34.5%
    #   和了率(鳴局)  20.6% / 20.2% / 16.8%
    #   放銃率(鳴局)   8.3% /  9.0% /  8.9%
    # 500局の局単位でも、副露した人数 861→502(−42%) に対して
    # 和了数は 373→378 とほぼ不変。減らした鳴きは和了を生んでいなかった。
    #
    ...
    # 和了る手段がゼロの副露手では、脅威の確度で 1.0 と混ぜない。
    #
    # **既定は False。順位で測ると負けたため。**
    #
    # 理屈は正しく見えた。和了る手段がゼロの手を押しても得るものが無いので、
    # 脅威の確度で「必ず押す」と混ぜるのはおかしい。決定単位でも素点でも
    # 良い数字が出た（副露の役なしテンパイ押しが 0.0% になり、素点 +45.5、
    # 和了 +0.38pt、放銃 -0.60pt）。
    #
    # それでも半荘・順位（各4000半荘、2対2、席と起家を入れ替え）では負けた。
    #   平均順位 2.5227 対 2.4773  → +0.0455（混ぜないほうが不利）
    #   トップ率 24.2% 対 25.8%、ラス率 25.9% 対 24.1%
    #
    # これは降りる方向の変更で、この環境では押し引きを降りに寄せると
    # 順位が落ちる（push 閾値の実測と同じ軸）。和了れない手でも、打牌は
    # 役に向かって進んでいるし（実測で29%が最終的に和了）、降りれば
    # そのぶん他家が和了りやすくなる。理屈は間違っていないが足りなかった。
    no_yaku_no_blend: bool = False
    riichi_ev_veto: bool = False
    riichi_ev_margin: int = 200   # ダマがこれだけ上回っていたら止める（点）
    # これ以上ダマで打点があればリーチしない。
    #
    # **この値は素点で決めてはいけない。** 固定25000の単独局で測ると
    # 12000 が 8000/5200/99999 の3通りすべてに勝った（素点 +31.1 など）が、
    # 半荘・順位で測り直すと逆だった。
    #
    #   12000 対 8000（各4000半荘、2対2、席と起家を入れ替え）
    #     平均順位 2.5067 対 2.4933  → +0.0135（12000が不利）
    #     ラス率   25.6% 対 24.4%    → +1.2pt（12000が不利）
    #
    # 押し寄りの変更は素点の平均を上げて分散も上げる。玉の間の目的は
    # ラス回避なので、単独局の素点では見えない（experiment-metric-failure.md）。
    # 差は約1SEで決定的ではないが、素点側の根拠が壊れた物差しのものなので
    # 既定は 8000 に戻す。
    damaten_value: int = 8000
...
class Player:
    ...
    def push_value(self, view, threat_level: float) -> float:
        ...
        # 脅威の確度で 1.0（＝必ず押す）と混ぜる。相手がリーチなら level 1.0 で
        # そのまま、副露読みだけなら 0.5 前後なので「よほど絶望的でなければ押す」
        # に落ちる。
        #
        # ただし **和了る手段がゼロの副露手** では、この混ぜ方が壊れる。
        # 混ぜる意味は「脅威は嘘かもしれないし、この手は進める価値がある」だが、
        # 後半が成り立たない。実際 level 0.5 では
        #   0.5 * 0.12 + 0.5 * 1.0 = 0.56 → 押し率ほぼ100%
        # になり、和了れない手で押し続けていた（残っていた押しの97%がこれ）。
        if not open_yaku and self.style.no_yaku_no_blend:
            return v
