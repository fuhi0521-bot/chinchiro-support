...
# 無作為化して測り直した表。
#
# tenpai_table.json は **エージェント自身の判断が作った状態** を数えたもので、
# 同じ (待ち枚数, 残り巡数) のセルの中でも、リーチした手とダマにした手は
# 打点も形も違う。打ち手は「手が悪いときだけダマにしていた」ので、
# ダマ側の成績が実際より低く出ていた。
#
# リーチの可否を 1/2 の確率で決めて測り直すと、セルの中で両側の分布が揃う。
# 差の出方（3000局・13617標本）:
#
#   6-8枚 残6-8巡   +2247 → +1285  （リーチ有利だが半分以下）
#   2-3枚 残6-8巡    +237 →  -437  （ダマ有利に逆転）
#   6-8枚 残0-2巡    +901 →  -311  （ダマ有利に逆転）
#
# 偏りのある表をそのまま使っていたら、リーチを過剰に勧める打ち手になっていた。
RANDOMIZED_TABLE = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                "data", "tenpai_table_rand.json")
_cache = {}
