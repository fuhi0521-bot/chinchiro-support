...
#
# ［再測定 700局・123950標本・フリテンの相手を除外］
#   数牌 0枚 9.79% / 1枚 6.94% / 2枚 3.48% / 3枚 2.45%
#   字牌 0枚 2.94% / 1枚 0.85% / 2枚 0.00% / 3枚 0.00%
#
# 2枚・3枚切れが上の値より **ずっと安全**だった。フリテン除外のせいではない
# （--keep-furiten でも 9.68/6.99/3.47/2.40 でほぼ同じ）。方策がその後
# 大きく変わった（役の門・タンヤオの門・現物優先の降り）ぶんのずれ。
#
# 生牌を1.00としたときの比:
#   いまの係数 1.00 / 0.75 / 0.56 / 0.44
#   素の実測   1.00 / 0.71 / 0.36 / 0.25
#
# ［却下］実測の比に合わせて (1.12, 0.79, 0.40, 0.28) にしたら **悪くなった**。
# 同じ700局で測り直すと:
#
#   枚数      実測(旧係数)  実測(新係数)   新係数での予測
#   2枚切れ      3.48%   →   3.85%         1.77%
#   3枚切れ      2.45%   →   2.83%         0.94%
#   AUC         0.6344  →   0.6316
#
# **安全と見積もるほど、その牌の当たり率が上がる。** 係数を下げると
# エージェントはその牌を危ない場面まで抱えて切るようになるので、
# 条件付きの当たり率が上がってくる。予測0.94%に対して実測2.08%まで開いた。
#
# つまりここで要るのは素の実測値ではなく、**その係数が作る方策のもとで
# 予測と実測が一致する不動点**。旧い値はほぼその不動点にあった
# （3枚切れ 予測1.62% / 実測1.77%）。素の実測に合わせるのは誤り。
#
...
# 字牌は数牌よりはっきり効く。2枚切れ以上は 2272標本で当たり **0件**。
# ロンに限れば事実上の安全牌だが、0 にすると現物と同じ扱いになってしまうので
# 下限だけ残す（ツモ和了・国士は別の話）。
#
# 数牌と違って、こちらは下げても不動点が動かなかった。係数を 0.10 → 0.05 に
# 下げてエージェントがより多く切るようになった後でも、2枚切れ1588標本・
# 3枚切れ708標本で当たりは **0件のまま**。選択の効果で上がってこない。
# 字牌の場合はシャンポン・単騎しか残らないという理屈が強いため。
HONOR_SEEN_FACTOR = (1.00, 0.29, 0.05, 0.03)
