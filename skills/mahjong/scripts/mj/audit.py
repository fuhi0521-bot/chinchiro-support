        ...
        if threats:
            ...
            if folding:
                ...
                if sh <= 0:
                    ...
                    s["1シャンテンから降りた"] += 1
                # --- 降りの切り順が正しいか。2つ数える。
                # _fold_discard は「安全度が同程度なら受け入れの広いほう」で決める。
                # 安全牌の在庫を一切見ていないので、次の2つが起こりうる。
                if safe:
                    # (1) 全脅威に通る牌を持っているのに、通らない牌を切った
                    if tile not in safe:
                        s["降り・現物があるのに切らず"] += 1
                    # (2) 唯一の安全牌を使い切って、次巡は危険牌しか残らない
                    elif sum(1 for t in safe if t != tile or hand[t] > 1) == 0:
                        s["降り・在庫を使い切り"] += 1
            else:
                s["押し打牌"] += 1
                if not safe:
...
def report(audit) -> str:
    ...
    out.append("\n■ 降りの切り順（安全牌を持っていた降り打牌が母数）")
    out.append(f"  {'雀士':<10} {'降り打牌':>9} {'現物を切らず':>13} {'在庫使い切り':>13}")
    out.append("-" * 50)
    for name, s2 in audit.stat.items():
        nd = s2["降り打牌"] or 1
        a = s2["降り・現物があるのに切らず"]
        b = s2["降り・在庫を使い切り"]
        out.append(f"  {name:<10} {s2['降り打牌']:>9} {a:>8} {a / nd * 100:>4.1f}% "
                   f"{b:>8} {b / nd * 100:>4.1f}%")
    out.append("")
    out.append("  現物を切らず ＝ 全脅威に通る牌を持っていたのに、通らない牌を切った")
