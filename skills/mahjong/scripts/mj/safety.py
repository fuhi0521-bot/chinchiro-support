...
#
# ［注意］この値は `reading.SEEN_FACTOR`（1.00/0.75/0.56/0.44 相当）より
# 急な坂になっている。わざとで、使い道が違う。
#   - ここは「いま手の中にあるこの牌は、切ったらどれくらい当たるか」を
#     人が読むための表。素の実測がそのまま答えになる。
#   - SEEN_FACTOR はエージェントが**選ぶために**使う。安全と見積もるほど
#     その牌を危ない場面まで抱えて切るので、条件付きの当たり率が上がる。
#     素の実測に合わせると予測と実測が開く（reading.py の該当箇所を参照）。
# 序列は両者で変わらないので、実戦の使い勝手には響かない。
