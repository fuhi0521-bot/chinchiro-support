...
このリポジトリの数字は3種類ある。混ぜて読まないこと。

1. **このエンジンで計算・実測した値** — 配牌のシャンテン分布、有効牌を引ける確率、
   シミュレーションの全結果。再現できる
2. **外部の統計と照合した値** — 流局率、和了率、放銃率、副露率、立直率、配牌の平均向聴数。
   出典は `skills/mahjong/references/probability.md` の末尾に列挙
3. **出典を確認していない値** — 押し引きの基準表、危険度の放銃率、平均和了打点、
   役の出現率など。麻雀の解説で広く使われている数字をそのまま書いたもので、
   **このリポジトリが検証したものではない**

3 は判断の方向性を決めるには使えるが、**細かい差を根拠に結論を出してはいけない**。
